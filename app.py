...
client = MongoClient("mongodb+srv://l2s_user:ltos_yous3R@l2c.xzbms.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
    ...
    if request.method == 'POST' and 'name' in request.form and 'email' in request.form and 'password' in request.form and 'cpassword' in request.form:
        ...
        if (exist is None):
            if (request.form['password'] == request.form['cpassword']):
                ...
                data.insert({'_id' : uuid.uuid4().hex,'name' : request.form['name'],'email' : request.form['email'], 'password' : hashedpass})
...
def login():
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form:
        ...
        if exist_user:
            if bcrypt.hashpw(request.form['password'].encode('utf-8'), exist_user['password']) == exist_user['password']:
